...
<code> if ( digitalRead(8) == true)
  stopSaw();
 </code> , right? **No!** Probaly there is a <code>for()</code> loop or a <code> while()</code> in your code that takes some time to analyse. What if the processor is to busy running that loop that it forgots to check if there is a hand next to the saw? Oops, somebody is going to get damaged and somebody is going to be fired.
